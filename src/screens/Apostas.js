...
export default () => {
  ...
  const [Cambis, setCambis] = useState(true);
...
useEffect(() => {
  ...
  useEffect(() => {
    console.log(userState.JogosApos)
    setDadoTitu(userState.QN4)
     setDadoFili(userState.QN3)
     setDadosBet(userState.versaoBanco)
     setJogAPos(userState.JogosApos)
  }, [userState.QN4, userState.QN3, userState.versaoBanco, userState.JogosApos])
  ...
  const PegandoLig = ()=>{
   ...
   const ListandoOc = ()=>{
    ...
    if(Dia === ""){
       ...
       let Mon = moment(dataNasc).format('DD')
        console.log(Mon);
        let Dia1 = meg[0];
        let Mes1 =  meg[1];
        let Ano1 = meg[2];
  ...
  const onChangeRec = ()=> {
   ...
   const ColocarNota=( item3, item)=>{
    console.log(item)
    console.log(item3)
    if(userState.QN4.QuanApos > userState.JogosApos || userState.QN4.Funcionario === false){
    if(item3.Olds){
        ...
        const AposCambis = ()=>{
          ...
          if(userState.QN4.Funcionario === true){
            ...
            if(userState.QN3.DataVenc >= DataMin){
              if(userState.QN3.Dinheiro <= userState.versaoBanco.ValMinSalDev){
                ...
                setModalCalend(true);
              setVerNotajogo(false);
              setAlert(`Você Não Pode Criar Uma Aposta, Pois sua Conta está desativada devido a Seu Saldo Devedor, passou o valor máximo permitido (R$${userState.versaoBanco.ValMinSalDev}). Vá ao financeiro e pague seu saldo devedor!  `);
              setAlertTipo("danger");
              }
              ...
              setModalCalend(true);
            setVerNotajogo(false);
            setAlert("Você Não Pode Criar Uma Aposta, Pois sua Conta está desativada devido a data de Vencimento da Conta. Vá ao financeiro e atualize a data de vencimento!   ");
            setAlertTipo("danger");
            }
            ...
            if(userState.QN4.DataVenc >= DataMin){
              if(userState.QN4.Dinheiro <= DadosBet.ValMinSalDev){
                ...
                setModalCalend(true);
              setVerNotajogo(false);
              setAlert(`Você Não Pode Criar Uma Aposta, Pois sua Conta está desativada devido a Seu Saldo Devedor, passou o valor máximo permitido (R$${DadosBet.ValMinSalDev}).Vá ao financeiro e pague seu saldo devedor! `);
              setAlertTipo("danger");
              }
         ...
         const PagandoCash = ()=>{
          ...
          if(verSim.includes(1)){
            ...
            if(userState.QN4.Funcionario === true){
              if(ValPremi <= userState.QN3.ConfigEmp.PremioMax){
              if(ValorReal <= userState.QN3.ConfigEmp.ValMax){
                if(ValorReal >= userState.QN3.ConfigEmp.ValMin){
                  if(SimAp.length >= userState.QN3.ConfigEmp.PalMin){
                    ...
                    setModalCalend(true);
                  setVerNotajogo(false);
                    setAlert(`${userState.QN3.ConfigEmp.PalMax} palpites são o máximo para aprovar uma aposta`);
                    setAlertTipo("danger");
              ...
              setModalCalend(true);
              setVerNotajogo(false);
              setAlert(`R$ ${userState.QN3.ConfigEmp.PremioMax} é o Valor Máximo que pode ser pago Pelo Prêmio!`);
              setAlertTipo("danger");
              ...
              if(ValPremi <= userState.QN4.ConfigEmp.PremioMax){
              if(ValorReal <= userState.QN4.ConfigEmp.ValMax){
                if(ValorReal >= userState.QN4.ConfigEmp.ValMin){
                  if(SimAp.length >= userState.QN4.ConfigEmp.PalMin){
                    ...
                    setModalCalend(true);
                  setVerNotajogo(false);
                    setAlert(`${userState.QN4.ConfigEmp.PalMax} palpites são o máximo para aprovar uma aposta`);
                    setAlertTipo("danger");
              ...
              setModalCalend(true);
              setVerNotajogo(false);
              setAlert(`R$ ${userState.QN4.ConfigEmp.PremioMax} é o Valor Máximo que pode ser pago Pelo Prêmio!`);
              setAlertTipo("danger");
              ...
              <>
                ...
                <Image source={require('../assets/futebol.gif')}  style={styles.ImageVer5 } />     
                   ...
                   <FontAwesome name="expeditedssl" size={40} color="black" />           
                       ...
                       <TouchableHighlight style={{width:150, height:50, backgroundColor:"#9B1AD3", borderRadius:5, margin:20, flex:1, justifyContent:"center", alignItems:"center" }} onPress={()=>CriandoApostaFilt()}>
