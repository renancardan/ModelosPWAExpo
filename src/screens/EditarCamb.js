...
export default (routes) => {
 ...
 console.log(routes.route.params)
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    setPorComis(routes.route.params.Comissao)
    setQMAp(routes.route.params.QuanApos)
    setPorPremi(routes.route.params.PorcenPremio)
    setNomeCli(routes.route.params.Nome)
    setTelCli(routes.route.params.Tel)
    setIdCam(routes.route.params.Id)
   }, [])
  ...
  //  Api.AnaliseTelIndic(TelCli, setTe1, setNomeCli, setBtn1, setTe2, setBtn2, setDateInd)  
      ...
      const EnviandoMsg = () => {
        ...
        if(TelMsg === true){
            if(TelCli !== '' && NomeCli !== "") {
                setLoading(true);
                Api.EditandoCam(IdCam, NomeCli, QMAp, PorPremi, PorComis, LinWhats, Logo, TelCli, setAlertTipo, setAlert, setLoading, setModalCalend,  setBtn, setBtn1, setBtn2, setCodG, setSenha, setTentativa, setRobo, setNomeCli, setTelCli, setCodLast, setDateInd, setTelMsg, setTe1, setTe2);
